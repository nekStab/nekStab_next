      ...
      !end if
               if (isnan(alpha)) then
                  if (nid == 0) write (6, *) 'NaN detected in dot product'
                  call nek_end
               end if
            end select
            return
      ...
      ! Complex prodcedures definitions.
         ...
         subroutine cmplx_copy(out, from)
            ...
            call nopcopy(out%re%vx, out%re%vy, out%re%vz, out%re%pr, out%re%t,
     $   from%re%vx, from%re%vy, from%re%vz, from%re%pr, from%re%t)
            call nopcopy(out%im%vx, out%im%vy, out%im%vz, out%im%pr, out%im%t,
     $   from%im%vx, from%im%vy, from%im%vz, from%im%pr, from%im%t)
            out%time = from%time
         ...
         subroutine cmplx_zero(self)
            ...
            call noprzero(self%re%vx, self%re%vy, self%re%vz, self%re%pr, self%re%t)
            call noprzero(self%im%vx, self%im%vy, self%im%vz, self%im%pr, self%im%t)
            self%time = 0.0d0
            return
         ...
         real function cmplx_dot(self, vec) result(alpha)
            ...
            type is (cmplx_nek_vector)
               alpha = real_dot(self%re, vec%re)
               alpha = alpha + real_dot(self%im, vec%im)
            end select
         ...
         subroutine cmplx_scal(self, alpha)
            ...
            class(cmplx_nek_vector), intent(inout) :: self
            real, intent(in) :: alpha
            call nopcmult(self%re%vx, self%re%vy, self%re%vz, self%re%pr, self%re%t, alpha)
            call nopcmult(self%im%vx, self%im%vy, self%im%vz, self%im%pr, self%im%t, alpha)
            self%time = self%time*alpha
         ...
         subroutine cmplx_axpby(self, alpha, vec, beta)
            ...
            select type (vec)
            type is (cmplx_nek_vector)
               call nopaxpby(self%re%vx, self%re%vy, self%re%vz, self%re%pr, self%re%t, alpha,
     $   vec%re%vx, vec%re%vy, vec%re%vz, vec%re%pr, vec%re%t, beta)
               call nopaxpby(self%im%vx, self%im%vy, self%im%vz, self%im%pr, self%im%t, alpha,
     $   vec%im%vx, vec%im%vy, vec%im%vz, vec%im%pr, vec%im%t, beta)
            end select
            return
