      ...
      contains
         ...
         subroutine linear_stability_analysis(solver_mode, solver_type)
            implicit none
            include 'SIZE'
            include 'TOTAL'
      
            character(len=*), intent(in) :: solver_mode
            ...
            if (index(solver_mode, 'steady') > 0) then
      
               if (index(solver_type, 'direct') > 0 .or. index(solver_type, 'forward') > 0) then
                  if (nid == 0) write (*, *) 'Direct solver in steady mode.'
                  isDirect = .true.; call bcast(isDirect, lsize)
                  evop = 'd'; transpose = .false.
                  if (findiff_order > 1) evop = 'f'
               elseif (index(solver_type, 'adjoint') > 0 .or. index(solver_type, 'backward') > 0) then
                  if (nid == 0) write (*, *) 'Adjoint solver in steady mode.'
                  isAdjoint = .true.
                  evop = 'a'; transpose = .true.
               end if
      
            elseif (index(solver_mode, 'periodic') > 0) then
      
               if (index(solver_type, 'direct') > 0 .or. index(solver_type, 'forward') > 0) then
                  if (nid == 0) write (*, *) 'Direct solver in periodic mode.'
                  isFloquetDirect = .true.
                  evop = 'd'; transpose = .false.
                  if (findiff_order > 1) evop = 'f'
               elseif (index(solver_type, 'adjoint') > 0 .or. index(solver_type, 'backward') > 0) then
                  if (nid == 0) write (*, *) 'Adjoint solver in periodic mode.'
            ...
            do i = 1, size(X); call X(i)%zero(); end do
            call prepare_seed(X)
      
            if (nid == 0) write (*, *) 'Linear stability started: k_dim, schur_target, eigen_tol = ', k_dim, schur_tgt, eigen_tol
            call eigs(A, X, eigvecs, eigvals, residuals, info, verbosity=.false.,
     $   nev = schur_tgt, tolerance = eigen_tol, transpose = transpose)
            ...
            if (nid == 0) write (*, *) 'Linear stability finished.'
            isAdjoint = .false.; isDirect = .false.
            isFloquetAdjoint = .false.; isFloquetDirect = .false.
      
         end subroutine linear_stability_analysis
      
         subroutine transient_growth_analysis(solver_mode)
            implicit none
            include 'SIZE'
            include 'TOTAL'
            ...
            if (index(solver_mode, 'steady') > 0) then
               isTransientGrowth = .true.; isFloquetTransientGrowth = .false.
            elseif (index(solver_mode, 'periodic') > 0) then
               isFloquetTransientGrowth = .true.; isTransientGrowth = .false.
            end if
            ...
            do i = 1, size(U); call U(i)%zero(); call V(i)%zero(); end do
            call prepare_seed(U)
         ...
         subroutine resolvent_analysis(solver_mode)
            ...
            if (index(solver_mode, 'steady') > 0) then
               isResolvent = .true.; isFloquetResolvent = .false.
            elseif (index(solver_mode, 'periodic') > 0) then
               isFloquetResolvent = .true.; isResolvent = .false.
            end if
            ...
            sigma(:) = 0.0d0; uvecs(:, :) = 0.0d0; vvecs(:, :) = 0.0d0; residuals(:) = 0.0d0
            do i = 1, size(U); call U(i)%zero(); call V(i)%zero(); end do
            call prepare_seed(U%re); call prepare_seed(U%im)
            call prepare_seed(V%re); call prepare_seed(V%im)
      
            evop = 'r'
            if (nid == 0) write (*, *) 'Resolvent started: k_dim, schur_target, eigen_tol = ', k_dim, schur_tgt, eigen_tol
            call svds(R, U, V, uvecs, vvecs, sigma, residuals, info,
     $   nev = schur_tgt, tolerance = eigen_tol, verbosity = .true.)
            sigma = sigma**2
      
            call outpost_singvals(sigma(:), residuals(:), 'Spectrum_S'//trim(evop)//'.dat')
      !     call outpost_singvectors(U%real, V%real, uvecs, vvecs, sigma, residuals)
      
            if (nid == 0) write (*, *) 'Resolvent finished.'
            isResolvent = .false.; isFloquetResolvent = .false.
      ...
      ! Update UPO period from base flow file if Floquet mode is activated
            if (istep == 0 .and. (isFloquetDirect .or. isFloquetAdjoint .or.
     $   isFloquetTransientGrowth .or. isFloquetResolvent)) then
            param(10) = time
            if (nid == 0) then
      ...
      ! Recompute time steps and endTime to match UPO period
            if (param(10) > 0.0d0) then
               ...
               if (nid == 0) write (*, *) ' current CFL and target=', ctarg, param(26)
            ...
            subroutine prepare_seed(X)
               ...
               if (ifseed_nois) then ! noise as initial seed
                  ...
                  if (nid == 0) write (*, *) 'Filling fields with noise...'
