      ...
      ! here we add harmonic forcing for Resolvent case
         if (omega_t /= 0.0d0) then
         if (omega_t > 0.0d0) then ! (real part)
      
               ffx = ffx + fR_Re_u(ix, iy, iz, iel)*cos(omega_t) - fR_Im_u(ix, iy, iz, iel)*sin(omega_t)
               ffy = ffy + fR_Re_v(ix, iy, iz, iel)*cos(omega_t) - fR_Im_v(ix, iy, iz, iel)*sin(omega_t)
               if (if3D) ffz = ffz + fR_Re_w(ix, iy, iz, iel)*cos(omega_t) - fR_Im_w(ix, iy, iz, iel)*sin(omega_t)
         
            else ! omega_t < 0 (imag part)
         
               ffx = ffx + fR_Re_u(ix, iy, iz, iel)*sin(omega_t) + fR_Im_u(ix, iy, iz, iel)*cos(omega_t)
               ffy = ffy + fR_Re_v(ix, iy, iz, iel)*sin(omega_t) + fR_Im_v(ix, iy, iz, iel)*cos(omega_t)
               if (if3D) ffz = ffz + fR_Re_w(ix, iy, iz, iel)*sin(omega_t) + fR_Im_w(ix, iy, iz, iel)*cos(omega_t)
