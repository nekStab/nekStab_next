...
!---  > Outpost initial condition.
      if(istep.eq.0) call outpost2(vx,vy,vz,pr,t,nof,'   ')
...
!     cases can have CFL > 1 in initial time steps
      ...
      if (istep.gt.0 .and. lastep.eq.0 .and. iftran) then
         ...
         if (mod(istep,10).eq.0) then
            ...
            print *,''
            write(6,"('      Mean time per timestep: ',F8.4,'  dev:',I8,'ms')") ttime / istep, 
     $       nint(((ttime / istep) - ttime_stp) * 1000)
            write(6,"('      Estimated remaining time: ',I8,' hr ',I2,' min')") int(tmiss), nint((tmiss - int(tmiss)) * 60)
            ...
            write(6,"('      Current local time: ',F10.4,'  File:',I8)") time - deltatime, int((time - deltatime) / param(14)) + 1
            print *,''
         endif
      endif
...
c-----------------------------------------------------------------------
      ...
      if (mod(istep,skip).eq.0) then
         ...
         if (istep.lt.1) call cfill(vdiff,param(2),nv)
